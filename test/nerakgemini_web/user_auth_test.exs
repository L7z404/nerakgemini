defmodule NerakgeminiWeb.UserAuthTest do
  use NerakgeminiWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Nerakgemini.Accounts
  alias Nerakgemini.Accounts.Scope
  alias NerakgeminiWeb.UserAuth

  import Nerakgemini.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, NerakgeminiWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      NerakgeminiWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        NerakgeminiWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
